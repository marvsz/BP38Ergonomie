    ...
    tls(false)
{
    ...
    commandSocket.setProtocol(QSsl::TlsV1_0);
    commandSocket.setPeerVerifyMode(QSslSocket::VerifyNone);
}
...
void QFtpPI::sslErrors(const QList<QSslError> &errors){
    ...
    for(int i = 0; i < errors.size(); ++i){
        e.append((errors.at(i).errorString()) + "\n");
    }
...
//    qDebug("QFtpPI state: %d [processReply() intermediate]", state);
    ...
    if (rawCommand) {
        ...
        dtp.writeData();
    } else if(replyCodeInt == 234 && tls){
        commandSocket.startClientEncryption();
    } else if(replyCodeInt == 235 && tls){
        state = Failure;
    }
